/* ============================================================
   ...
   Vista: selector de AÑO + chips de GRUPO aditivos
   (En cartera · Con informe · En plan · Con dividendo · Sin
   dividendo). Listado ordenado por prioridad; fila plegada
   (empresa · dividendo total BRUTO · RPD vs cotización actual ·
   junta) y desplegada (pagos + presentación de resultados).
   ============================================================ */
...
function _evoInformeSet(){ var s=new Set(); (DB.analisis||[]).forEach(function(a){ if(a&&a.ticker) s.add((a.ticker||'').toUpperCase()); }); return s; }
function _evoPlanSet(){ var s=new Set(); var pc=DB.planCompras||{}; Object.keys(pc).forEach(function(t){ var any=Object.keys(pc[t]||{}).some(function(y){ return num(pc[t][y])>0; }); if(any) s.add((t||'').toUpperCase()); }); return s; }
...
function renderEvoDiv(){
  ...
  if(_evoYear==null){ _evoYear = (years.indexOf(nowY)>=0)?nowY:(years.length?years[years.length-1]:nowY); }
  ...
  var rows=empresas.map(function(e){
    ...
    var a=(e.anios&&e.anios[String(_evoYear)])?e.anios[String(_evoYear)]:null;
    var dpaB=a?(a.dpaBruto!=null?num(a.dpaBruto):null):null;
    var precio=_evoPrecio(t);
    var rpd=(dpaB!=null&&precio>0)?(dpaB/precio*100):null;
    ...
    return {e:e,t:t,a:a,dpaB:dpaB,precio:precio,rpd:rpd,paga:paga,
  ...
  /* orden: rank, luego RPD desc (o dividendo desc), luego nombre */
  ...
  var kpis='<div class="cards">'
    ...
    +card('Pagan en '+_evoYear, String(pag.length))
    +card('RPD media', _evoPf(rpdMed,2)+'%')
    +card('Actualizado', _evoEsc((_evoData.actualizado||'—')))
    +'</div>';

  /* selector de año + chips de grupo + buscador */
  var yearSel='<select id="evoYearSel" style="margin-left:6px;padding:4px 8px;border:1px solid var(--line);border-radius:6px;font-size:13px">'
    +years.slice().reverse().map(function(y){ return '<option value="'+y+'"'+(y===_evoYear?' selected':'')+'>'+y+(y>nowY?' · prev':'')+'</option>'; }).join('')
    +'</select>';
  ...
  var tabla='<div style="overflow:auto"><table><thead><tr>'
    ...
    +'<th class="num">Dividendo total (bruto)</th>'
    +'<th class="num">RPD vs cotización</th>'
    +'<th>Junta</th></tr></thead><tbody>'+trs+'</tbody></table></div>';

  var nota='<div class="muted" style="font-size:11px;margin-top:8px">Dividendo total y RPD en <b>bruto</b> (RPD = dividendo bruto ÷ cotización actual). '
  ...
  host.innerHTML='<div style="font-size:20px;font-weight:800;color:#1f3d6b;margin-bottom:2px">📅 Evolución del Dividendo</div>'
    +'<div class="muted" style="font-size:13px;margin-bottom:6px">Base de datos de dividendos por empresa y año. Elige el año y los grupos a mostrar.</div>'
    +kpis+toolbar+tabla+nota;
  ...
  host.querySelectorAll('tr.evo-main').forEach(function(tr){ tr.addEventListener('click',function(e){
    if(e.target.closest('[data-ficha]')) return;   /* el clic en el ticker abre la ficha, no despliega */
