...
const GROUPS={
  ...
  planinv:[['asignacion','Asignación'],['proxcompra','Próxima compra'],['riesgo','Riesgo'],['coyuntura','Coyuntura'],['simulador','Simulador'],['independencia','Independencia'],['pignorado','Capital pignorado']],
...
$('#view-panel').addEventListener('click',e=>{
  ...
  const h=e.target.closest('[data-goto]'); if(!h)return; if(h.dataset.sig&&typeof showProtocolo==='function'){ showProtocolo(h.dataset.sig,h.dataset.goto,h.dataset.ticker||'',h.dataset.nivel||''); return; } if(typeof activarVista==='function') activarVista(h.dataset.goto); });
if($('#view-patrimonio')){
  $('#view-patrimonio').addEventListener('click',e=>{ const a=e.target.closest('#asignAdd'); if(a){ if(typeof addClase==='function')addClase(); return; } const au=e.target.closest('#asignAuto'); if(au){ if(typeof autorellenarAsignacion==='function')autorellenarAsignacion(); return; } const sv=e.target.closest('#asignFotoSave'); if(sv){ if(typeof guardarFotoAsign==='function')guardarFotoAsign(); return; } const fd=e.target.closest('[data-asignfotodel]'); if(fd){ const _fid=fd.dataset.asignfotodel;
      const _ix=(DB.asignacionFotos||[]).findIndex(x=>x.id===_fid); if(_ix<0)return; const _it=DB.asignacionFotos[_ix];
      const _q=()=>{ DB.asignacionFotos=(DB.asignacionFotos||[]).filter(x=>x.id!==_fid); saveNow(); };
      if(typeof undoableDelete==='function') undoableDelete('asignFoto','Foto de asignación'+(_it.fecha?(' · '+_it.fecha):''),{item:_it,idx:_ix},_q,['renderAsignFotos']);
      else { if(confirm('¿Eliminar esta foto de asignación?')){ _q(); if(typeof renderAsignFotos==='function')renderAsignFotos(); } }
      return; } const d=e.target.closest('[data-asigndel]'); if(d){ const _aid=d.dataset.asigndel;
      const _ix=(DB.asignacion||[]).findIndex(x=>x.id===_aid); if(_ix<0)return; const _it=DB.asignacion[_ix];
      const _q=()=>{ DB.asignacion=(DB.asignacion||[]).filter(x=>x.id!==_aid); saveNow(); };
      if(typeof undoableDelete==='function') undoableDelete('asignClase','Clase de activo: '+((_it.nombre||'sin nombre')+''),{item:_it,idx:_ix},_q,['renderAsignacion','renderPanel']);
      else { if(confirm('¿Eliminar esta clase de activo?')){ _q(); if(typeof renderAsignacion==='function')renderAsignacion(); if(typeof renderPanel==='function')renderPanel(); } }
      return; } });
  $('#view-patrimonio').addEventListener('change',e=>{ const t=e.target.closest('[data-asign]'); if(!t)return; const a=t.dataset.asign.split('|'); const c=(DB.asignacion||[]).find(x=>x.id===a[0]); if(!c)return; const f=a[1]; if(f==='nombre')c[f]=t.value; else c[f]=(t.value.trim()===''?0:num(t.value)); saveNow(); if(typeof renderAsignacion==='function')renderAsignacion(); if(typeof renderPanel==='function')renderPanel(); });
  $('#view-presupuesto').addEventListener('click',e=>{ const a=e.target.closest('#metaAdd'); if(a){ if(typeof addMeta==='function')addMeta(); return; } /* [27-ago-2026] Metas, clases de activo y fotos de asignación eran los tres últimos borrados con
       confirm genérico y sin papelera. La etiqueta nombra lo que se pierde y se guarda el índice. */
    const d=e.target.closest('[data-metadel]'); if(d){ const _mid=d.dataset.metadel;
      const _ix=(DB.metas||[]).findIndex(x=>x.id===_mid); if(_ix<0)return; const _it=DB.metas[_ix];
      const _obj=(num(_it.objetivo)>0)?(' · objetivo '+fmt(num(_it.objetivo))):'';
      const _q=()=>{ DB.metas=(DB.metas||[]).filter(x=>x.id!==_mid); saveNow(); };
      if(typeof undoableDelete==='function') undoableDelete('meta','Meta: '+((_it.nombre||'sin nombre')+'')+_obj,{item:_it,idx:_ix},_q,['renderMetas','renderPanel']);
      else { if(confirm('¿Eliminar esta meta?')){ _q(); if(typeof renderMetas==='function')renderMetas(); if(typeof renderPanel==='function')renderPanel(); } }
    } });
  $('#view-presupuesto').addEventListener('change',e=>{ const t=e.target.closest('[data-meta]'); if(!t)return; const a=t.dataset.meta.split('|'); const m=(DB.metas||[]).find(x=>x.id===a[0]); if(!m)return; const f=a[1]; if(f==='nombre'||f==='fecha')m[f]=t.value; else m[f]=(t.value.trim()===''?0:num(t.value)); saveNow(); if(typeof renderMetas==='function')renderMetas(); if(typeof renderPanel==='function')renderPanel(); });
}
if($('#view-rentabilidad')){
  $('#view-rentabilidad').addEventListener('click',e=>{ const b=e.target.closest('#blkAtrib [data-atrib]'); if(b){ atribSel.modo=b.dataset.atrib; if(typeof renderAtribucion==='function')renderAtribucion(); return; } const r=e.target.closest('#atribRango'); if(r){ const dd=$('#atribDesde').value,hh=$('#atribHasta').value; if(dd&&hh){ atribSel.modo='rango'; atribSel.desde=dd; atribSel.hasta=hh; if(typeof renderAtribucion==='function')renderAtribucion(); } else alert('Elige fecha de inicio y de fin.'); return; } });
  $('#view-rentabilidad').addEventListener('change',e=>{ const y=e.target.closest('#atribAnio'); if(y&&y.value){ atribSel.modo='anio'; atribSel.anio=+y.value; if(typeof renderAtribucion==='function')renderAtribucion(); } });
}
if($('#view-proxcompra')){
...
if($('#view-monitor'))$('#view-monitor').addEventListener('click',e=>{ const d=e.target.closest('[data-tododel]'); if(d){
    ...
    const _t=DB.todos[_ix];
    /* El campo es `desc` (así lo escribe #todoForm y así lo pinta renderMonitor). */
    const _lbl='Tarea: '+((_t.desc||'sin título')+'').slice(0,48)+(_t.ticker?(' · '+_t.ticker):'');
...
$('#loteTabla').addEventListener('change',e=>{ const dp=e.target.closest('[data-lotedisp]'); if(dp){ DB.planDispFijo=DB.planDispFijo||{}; if(dp.value.trim()==='')delete DB.planDispFijo[dp.dataset.lotedisp]; else DB.planDispFijo[dp.dataset.lotedisp]=num(dp.value); saveNow(); renderPlanLote(); return; } const ag=e.target.closest('[data-asig]'); if(ag){ const [t,y]=ag.dataset.asig.split('|'); DB.planCompras=DB.planCompras||{}; DB.planCompras[t]=DB.planCompras[t]||{}; if(ag.value.trim()==='')delete DB.planCompras[t][y]; else DB.planCompras[t][y]=num(ag.value); saveNow(); renderPlanLote();  if(typeof renderSimulador==='function')renderSimulador(); return; } const yr=e.target.closest('[data-loteyr]'); if(yr){ DB.planLotePeriodo=DB.planLotePeriodo||{}; DB.planLotePeriodo[yr.dataset.loteyr]=num(yr.value); saveNow(); renderPlanLote(); if(typeof renderPlan==='function')renderPlan(); if(typeof renderSimulador==='function')renderSimulador(); return; } const ad=e.target.closest('[data-loteadd]'); if(ad){ const tk=_loteTk(ad.value); if(tk){ DB.planLote=DB.planLote||[]; if(!DB.planLote.includes(tk))DB.planLote.push(tk); } saveNow(); renderPlanLote(); return; } /* [27-ago-2026] Retirado el handler de data-lotechg: ese atributo no lo emite nadie (0 usos en 04-plan.js
     e index.html) y hacía splice de DB.planLote sin limpiar planTipo/planCompras — el mismo fallo que
     arreglamos en la baja, esperando a que alguien reintrodujera el marcado. */ });
/* [25-ago-2026] La baja pasa por dvBajaEmpresa (04-plan.js): confirma, limpia planTipo/planCompras
   —sin eso la fila se reinyectaba sola— y deja deshacer en la Papelera. La rama numérica es del
   marcado antiguo (índice en vez de ticker) y se conserva por si queda algún resto. */
$('#loteTabla').addEventListener('click',e=>{ const d=e.target.closest('[data-lotedel]'); if(!d)return; const _tk=(d.dataset.lotedel||'').toUpperCase();
...
/* [09-sep-2026] La comisión se propone sola al teclear acciones o precio, y se calla en cuanto la
   tocas. Ver `opComisionSugerir` en 03-inversiones.js. */
['#opAcc','#opPrecio'].forEach(function(sel){ const el=$(sel); if(el) el.addEventListener('input',function(){ if(typeof opComisionSugerir==='function')opComisionSugerir(); }); });
(function(){ const c=$('#opComision'); if(c) c.addEventListener('input',function(){ c.dataset.tocado='1'; }); })();
...
document.addEventListener('change',e=>{ const t=e.target; if(!t||!t.dataset)return; const _rp=(typeof renderProyConScroll==='function')?renderProyConScroll:renderProy; if(t.id==='proyRealTgl'){ window._proyRealOn=t.checked; if(typeof renderProy==='function')_rp(); return; } if(t.dataset.proy){ let v=num(t.value); if(t.dataset.pct==='1') v=v/100; DB.config.proyeccion[t.dataset.proy]=v; _rp(); scheduleSave(); } else if(t.classList&&t.classList.contains('aporInput')){ DB.config.proyeccion.aportaciones[t.dataset.anio]=num(t.value); _rp(); scheduleSave(); } else if(t.classList&&t.classList.contains('gastoInput')){ DB.config.proyeccion.gastos=DB.config.proyeccion.gastos||{}; DB.config.proyeccion.gastos[t.dataset.anio]=num(t.value); _rp(); scheduleSave(); } else if(t.classList&&t.classList.contains('extraInput')){ DB.config.proyeccion.ingresosExtra=DB.config.proyeccion.ingresosExtra||{}; const _xv=num(t.value); if(_xv>0){ DB.config.proyeccion.ingresosExtra[t.dataset.anio]=_xv; } else { delete DB.config.proyeccion.ingresosExtra[t.dataset.anio]; } _rp(); scheduleSave(); } });
...
$('#invClosed').addEventListener('click',e=>{ const a=e.target.closest('[data-archive]'); if(a){ archivarCerrada(a.dataset.archive); return; } const d=e.target.closest('[data-delcerrada]'); if(d){ const _id=d.dataset.delcerrada; const _it=(DB.cerradas||[]).find(c=>c.id===_id); if(!_it)return;
  /* La ✕ BORRA DEFINITIVAMENTE: las operaciones se quitaron de la cartera al archivar y no vuelven.
     Por eso se detalla qué se pierde antes de confirmar (la papelera sigue siendo la red de 30 días). */
  const _t=_it.ticker||'', _ops=(_it.ops||[]), _dv=(_it.divs||[]);
  const _f=_ops.map(o=>o.fecha).filter(Boolean).sort(), _per=_f.length?(_f[0]+' → '+_f[_f.length-1]):'sin fechas';
  const _eur=_dv.length?(' ('+_dv.map(x=>x.fecha).filter(Boolean).sort()[0]+' → '+_dv.map(x=>x.fecha).filter(Boolean).sort().slice(-1)[0]+')'):'';
  if(!confirm('¿Borrar definitivamente la posición cerrada de '+_t+'?\n\nSe pierde el CICLO COMPLETO:\n · '+_ops.length+' operaciones ('+_per+')\n · '+_dv.length+' dividendos archivados'+_eur+'\n\nEstas operaciones NO vuelven a la cartera: se quitaron de ella al archivar.\nPodrás recuperarlo desde la Papelera durante 30 días.')) return;
  undoableDelete('cerrada','Posición archivada '+_t,{item:_it},()=>{ DB.cerradas=(DB.cerradas||[]).filter(c=>c.id!==_id); },['renderInv','renderDividendos']); } });

/* Los botones que hay a la derecha del selector de vistas cambian con el estado (medias y c/div en
   «Precio», la ventana en «Móvil», ninguno en las demás), así que además de redibujar el gráfico
   hay que repintar su propia barra. Se repinta solo ese <div class="seg">, no la ficha entera:
   mantiene el scroll y el zoom. */
function _fichaDivRepintar(){
  if(typeof _fichaSavePrefs==='function')_fichaSavePrefs();
  var c=document.getElementById('fchMod');
  if(c&&typeof _fichaModBtnsHTML==='function')c.innerHTML=_fichaModBtnsHTML();
  if(typeof drawFichaChart==='function'&&typeof fichaTicker!=='undefined'&&fichaTicker)drawFichaChart(fichaTicker);
}
$('#fichaView').addEventListener('click',e=>{ const _fr=e.target.closest('[data-frange]'); if(_fr){ fichaRange=_fr.dataset.frange; fichaZoom=null; if(_fr.parentElement)[..._fr.parentElement.children].forEach(x=>x.classList.toggle('on',x===_fr)); if(typeof _fichaSavePrefs==='function')_fichaSavePrefs(); drawFichaChart(fichaTicker); return; } const _fm=e.target.closest('[data-fma]'); if(_fm){ const _w=+_fm.dataset.fma; if(typeof fichaMA!=='undefined'){ fichaMA[_w]=!fichaMA[_w]; _fm.classList.toggle('on',!!fichaMA[_w]); } if(typeof _fichaSavePrefs==='function')_fichaSavePrefs(); if(typeof drawFichaChart==='function')drawFichaChart(fichaTicker); return; } const _fv=e.target.closest('[data-fvista]'); if(_fv){ if(typeof fichaVista!=='undefined')fichaVista=_fv.dataset.fvista; if(_fv.parentElement)[..._fv.parentElement.children].forEach(x=>x.classList.toggle('on',x===_fv)); var _tt=document.getElementById('fchTit'); if(_tt&&typeof _FICHA_VISTA_TIT!=='undefined')_tt.textContent=_FICHA_VISTA_TIT[fichaVista]||'Cotización'; _fichaDivRepintar(); return; } const _fr5=e.target.closest('[data-froll]'); if(_fr5){ if(typeof fichaRollN!=='undefined')fichaRollN=+_fr5.dataset.froll; _fichaDivRepintar(); return; } const _fcd=e.target.closest('[data-fcondiv]'); if(_fcd){ if(typeof fichaConDiv!=='undefined')fichaConDiv=!fichaConDiv; _fichaDivRepintar(); return; } const _fdm=e.target.closest('[data-fdivmet]'); if(_fdm){ if(typeof fichaDivMet!=='undefined')fichaDivMet=(fichaDivMet==='tr')?'sum':((fichaDivMet==='sum')?'both':'tr');   /* reinvertido → suma → las dos */ _fichaDivRepintar(); return; } const _fsc=e.target.closest('[data-fsolocot]'); if(_fsc){ if(typeof fichaSoloCot!=='undefined')fichaSoloCot=!fichaSoloCot; _fichaDivRepintar(); return; } const _fz=e.target.closest('[data-fzreset]'); if(_fz){ fichaZoom=null; if(typeof drawFichaChart==='function')drawFichaChart(fichaTicker); return; } if(e.target.closest('#finvVal')){ validarUrlInv(); return; } if(e.target.closest('#fdivAdd')){ addFichaDiv(); return; } const dd=e.target.closest('[data-deldiv]'); if(dd){ const _t=fichaTicker; const arr=(DB.dividendos||{})[_t]||[]; const i=arr.findIndex(x=>x.id===dd.dataset.deldiv); if(i>=0){ const _it=arr[i]; undoableDelete('dividendo','Dividendo '+_t+(_it.fecha?(' '+_it.fecha):''),{t:_t,item:_it},()=>{ arr.splice(i,1); },['renderDividendos']); } } });
