...
$('#loteTabla').addEventListener('click',e=>{ const d=e.target.closest('[data-lotedel]'); if(d){ const _tk=(d.dataset.lotedel||'').toUpperCase(); if(_tk&&isNaN(+_tk)){ DB.planLote=(DB.planLote||[]).filter(x=>(x||'').toUpperCase()!==_tk); } else { DB.planLote.splice(+_tk,1); } saveNow(); renderPlanLote(); } });
