...
function renderRanking(){
  ...
  const _sh=(k,l,c)=>`<th class="${c||''}" data-sorttbl="ranking" data-sortk="${k}" style="cursor:pointer" title="Ordenar">${l}${sortArrow('ranking',k)}</th>`; const head='<tr>'+_sh('ticker','Empresa','l')+_sh('divAno','Div/año')+_sh('pesoDiv','Peso div')+_sh('yoc','YoC')+_sh('valor','Valor')+_sh('bal','Balance')+_sh('totPct','Total%')+'</tr>';
  const body=rows.map(r=>`<tr class="mt-row"><td class="emp"><span class="mt-arw">▶</span><b class="rank-tk" data-ficha="${r.ticker}" style="cursor:pointer;color:var(--brand)">${r.ticker}</b> <span style="font-weight:600;color:#334155;font-size:11px">${r.nombre||''}</span></td><td class="mt-pos"><b>${fmt(r.divAno)}</b></td><td><span class="mt-pill b">${pct(r.pesoDiv)}</span></td><td>${pct(r.yoc)}</td><td>${fmt(r.valor)}</td><td><span class="${r.bal>=0?'mt-pos':'mt-neg'}" style="font-weight:700">${r.bal>=0?'+':''}${fmt(r.bal)}</span></td><td><span class="mt-pill ${r.totPct>=0?'g':'r'}">${pct(r.totPct)}</span></td></tr><tr class="mt-det"><td colspan="7"><div class="mt-nums">${_mtNum('Títulos',r.sh)}${_mtNum('P. medio',fmt(r.pm))}${_mtNum('Div/acción',fmt(r.divAcc))}${_mtNum('Coste',fmt(r.cost))}${_mtNum('Dividendos cobrados',fmt(r.cobr),'mt-pos')}${_mtNum('Cotización',fmt(r.cot))}${_mtNum('Revalorización',pct(r.reval),r.reval>=0?'mt-pos':'mt-neg')}${_mtNum('Div acum. %',pct(r.divPct),'mt-pos')}${_mtNum('%Coste',pct(r.pesoCoste))}${_mtNum('%Valor',pct(r.pesoValor))}${_mtNum('%Balance',pct(r.pesoBal))}</div></td></tr>`).join('');
  const foot=`<tr class="mt-sub"><td class="l">TOTAL</td><td class="mt-pos"><b>${fmt(T.divAno)}</b></td><td>100%</td><td>${pct(T.cost?T.divAno/T.cost:0)}</td><td>${fmt(T.valor)}</td><td class="${T.bal>=0?'mt-pos':'mt-neg'}">${T.bal>=0?'+':''}${fmt(T.bal)}</td><td><span class="mt-pill ${T.bal>=0?'g':'r'}">${pct(T.cost?T.bal/T.cost:0)}</span></td></tr>`;
  ...
  const tblBlk=`<div class="pos-desk"><div class="mt-wrap"><table class="mt-tbl"><thead>${head}</thead><tbody>${body}${foot}</tbody></table></div></div><div class="pos-mob">${mcards}</div>`;
...
function renderSimulador(){
  ...
  /* ===== Realidad de dividendos (pasado y año en curso) = lo REALMENTE cobrado por empresa y año.
     Suma abiertas (DB.dividendos, saltando cerrados) + cerradas.divs + divIngresos, con las acciones
     a la fecha de cada pago según TODAS las operaciones (_allOps, abiertas+cerradas). Dedup: para un
     ticker cerrado manda cerradas.divs. El FUTURO (y>nowY) sigue siendo proyección: acciones×DPA. ===== */
  const _rOps=(typeof _allOps==='function')?_allOps():(DB.operaciones||[]);
  const _rClosed={}; (DB.cerradas||[]).forEach(c=>{ _rClosed[(c.ticker||'').toUpperCase()]=1; });
  ...
  const body=tickers.map(t=>{ const real=simIsReal(t);
    const cells=years.map(y=>{ const fut=y>nowY; let accHTML,divHTML,impCell,dlHTML;
      if(fut){ const divRaw=simDpa(t,y); const sh=simEffShares(t,y,nowY); const imp=sh*num(divRaw||0);
        accHTML=`<span class="ro">${sh||'·'}</span>`;
        const divStar=divRaw===0?'<span style="color:#dc2626;font-weight:700">*</span>':'';
        divHTML=(divRaw==null?'·':num(divRaw).toFixed(3))+divStar;
        impCell=(divRaw===0&&sh>0)?'0 <span style="color:#dc2626;font-weight:700">*</span>':(imp>0?fmt(imp):'·');
        const cc=(DB.divConfirmado[t]&&DB.divConfirmado[t][y])||conf[y]; const dlConf=(cc&&!conf[y]);
        dlHTML=`<div class="c-dl ${dlConf?'conf':''}" data-divconf="${t}|${y}" title="Clic: marcar dividendo real confirmado">div '${String(y).slice(2)}${dlConf?' ✓':''}</div>`;
      } else { const rImp=simRealImp(t,y); const rSh=simRealSh(t,y); const rDiv=(rSh>0)?rImp/rSh:null;
        accHTML=`<span class="ro">${rSh||'·'}</span>`;
        divHTML=(rDiv==null?'·':rDiv.toFixed(3));
        impCell=rImp>0?fmt(rImp):'·';
        dlHTML=`<div class="c-dl" style="color:#94a3b8">div '${String(y).slice(2)}</div>`;
      }
      return `<td class="cell ${fut?'prev':''}"><div class="c-tk">${t}</div><div class="c-acc">${accHTML}</div>${dlHTML}<div class="c-dv">${divHTML}</div><div class="c-im">${impCell}</div></td>`;
    }).join('');
  ...
  const mrow=(t,y)=>{ const fut=y>nowY; let sh,divTxt,imp;
    if(fut){ const divRaw=simDpa(t,y); sh=simEffShares(t,y,nowY); imp=sh*num(divRaw||0); divTxt=(divRaw==null?'·':num(divRaw).toFixed(3)); if(!(sh>0)&&!(imp>0)) return ''; }
    else { const rImp=simRealImp(t,y); const rSh=simRealSh(t,y); sh=rSh; imp=rImp; divTxt=(rSh>0?(rImp/rSh).toFixed(3):'·'); if(!(rSh>0)&&!(imp>0)) return ''; }
    return `<div class="simrow"><span class="yy">${y}${fut?' <em>prev</em>':''}</span><div class="acc"><b>${sh||'·'}</b><span class="lbl">acc.</span></div><div class="dv">${divTxt}<span class="lbl">€/acc</span></div><div class="im">${imp>0?fmt(imp):'·'}<span class="lbl">importe</span></div></div>`; };
...
/* [25-ago-2026] _precioActualDe vive ahora en 01-core.js (fuente única, ver precioActualInfo).
   Aquí quedaba la única copia completa de la cadena de 3 niveles; Cartera no la usaba y por eso
   pintaba −100 % donde el Kanban pintaba el precio bien. */
...
function renderFiscalidad(){ const el=$('#fiscalBody'); if(!el)return; const kp=$('#fiscalKpis'); const F=fiscalidadData();
  ...
  const candRows=cand.map(l=>`<tr><td class="emp"><b class="fisc-tk" data-ficha="${l.t}" style="color:var(--brand)">${l.t}</b></td><td class="l">${l.fecha||'—'}</td><td>${(+l.acciones).toFixed(0)}</td><td>${f2(l.precio)}</td><td>${f2(l.precioActual)}</td><td class="mt-neg">${fmt(l.latente)}</td><td class="mt-neg">${l.pct!=null?(l.pct*100).toFixed(1)+'%':'—'}</td><td class="l" style="font-size:11px">${l.recompraReciente?'<span class="warn">⚠ compra &lt;2m: no deducible ahora</span>':'<span class="muted">ok si no recompras 2m</span>'}</td></tr>`).join('');
  const candDesk=`<div class="mt-wrap"><table class="mt-tbl"><thead><tr><th class="l">Empresa</th><th class="l">Compra</th><th>Acc</th><th>P.compra</th><th>P.actual</th><th>Minusvalía</th><th>%</th><th class="l">Regla 2 meses</th></tr></thead><tbody>${candRows}</tbody></table></div>`;
  ...
  let realBody='';
  win.filter(y=>r5[y]).forEach(y=>{ const rr=r5[y].rows.slice().sort((a,b)=>(b.fechaV||'').localeCompare(a.fechaV||'')); rr.forEach((r,i)=>{ realBody+=`<tr><td class="l">${i===0?('<b>'+y+'</b>'):''}</td><td class="l"><b class="fisc-tk" data-ficha="${r.t}" style="color:var(--brand)">${r.t}</b></td><td class="l">${r.fechaV||'—'}</td><td>${(+r.acc).toFixed(0)}</td><td>${f2(r.precioC)} → ${f2(r.precioV)}</td><td class="${r.gan>=0?'mt-pos':'mt-neg'}">${sg(r.gan)}</td></tr>`; }); realBody+=`<tr class="mt-sub"><td class="l" colspan="5">Neto ${y}</td><td class="${yNet(y)>=0?'mt-pos':'mt-neg'}">${sg(yNet(y))}</td></tr>`; });
  const realDesk=`<div class="mt-wrap"><table class="mt-tbl"><thead><tr><th class="l">Año</th><th class="l">Empresa</th><th class="l">Venta</th><th>Acc</th><th>Compra→Venta</th><th>Ganancia</th></tr></thead><tbody>${realBody}</tbody></table></div>`;
  let realMob='';
...
function renderRebalanceo(){
  ...
  const _ebMt=r=>r.estado==='bajo'?'<span class="mt-pill a">▼ infra</span>':(r.estado==='alto'?'<span class="mt-pill r">▲ sobre</span>':'<span class="mt-pill g">en banda</span>');
  const drows=rows.map(r=>`<tr><td class="emp"><b class="tk" data-ficha="${r.t}" style="cursor:pointer;color:var(--brand)">${r.t}</b> <span style="font-weight:600;color:#334155;font-size:11px">${(r.nombre||'').slice(0,18)}</span></td><td>${fmt(r.curV)}</td><td>${(r.curW*100).toFixed(1)}%</td><td>${(r.targetW*100).toFixed(1)}%</td><td class="${r.driftPP<0?'mt-pos':(r.driftPP>0?'mt-neg':'')}">${r.driftPP>=0?'+':''}${r.driftPP.toFixed(1)} pp</td><td class="c">${_ebMt(r)}</td><td>${r.aportar>0?'<b class="mt-pos">'+fmt(r.aportar)+'</b>':'<span class="mt-sub2">·</span>'}</td></tr>`).join('');
  const deskHTML='<div class="rb-desk"><div class="mt-wrap"><table class="mt-tbl"><thead><tr><th class="l">Empresa</th><th>Valor</th><th>% actual</th><th>% objetivo</th><th>Desvío</th><th class="c">Banda</th><th>Aportar €</th></tr></thead><tbody>'+drows+'</tbody></table></div></div>';
  ...
  try{ var FISC=(typeof fiscalidadData==='function')?fiscalidadData():null;
    if(FISC&&FISC.openLots){
      ...
      if(overR.length){
        ...
        var trs=overR.map(function(x,i){ return '<tr><td class="c">'+(i+1)+'</td><td class="l"><b class="tk" data-ficha="'+x.t+'" style="color:var(--brand)">'+x.t+'</b></td><td class="mt-neg">+'+x.driftPP.toFixed(1)+' pp</td><td>'+fmt(x.exceso)+'</td><td>'+x.soldAcc+'</td><td class="'+(x.real>=0?'mt-neg':'mt-pos')+'"><b>'+(x.real>=0?'+':'')+fmt(x.real)+'</b></td><td><span class="p7eff" style="color:'+eCol(x.effRatio)+'">'+(x.effRatio*100).toFixed(0)+'%</span></td><td><span class="p7efb" style="background:'+eCol(x.effRatio)+'">'+eLbl(x)+'</span></td></tr>'; }).join('');
        ...
        _p7='<div class="p7blk'+(window._rebP7Open?' open':'')+'"><div class="p7blk-h"><span class="arw">▶</span><span class="bt">✂️ Ajuste con venta (consciente de fiscalidad)</span><span class="bsum">'+overR.length+' sobreponderadas · peaje fiscal</span></div><div class="p7blk-b"><div class="p7pad">'
          ...
          +'<div class="p7desk"><div class="mt-wrap"><table class="mt-tbl"><thead><tr><th class="c">#</th><th class="l">Empresa</th><th>Sobre obj.</th><th>Recortar</th><th>Vender</th><th>Plusvalía</th><th>Peaje/€</th><th class="c">Nivel</th></tr></thead><tbody>'+trs+'</tbody></table></div></div>'
...
function proximaCompra(){ if(typeof cmpScore!=='function')return null;
  ...
  const items=(DB.analisis||[]).map(a=>{ const t=(a.ticker||'').toUpperCase(); const cot=(typeof precioDe==='function')?num(precioDe(a)):num(a.cotizacion),entMax=num(a.entMax),entMin=num(a.entMin),stop=num(a.stopTesis); const score=cmpScore(a); const dec=(a.decision||'').toUpperCase();
...
function backtestData(){ const TH=DB.tesisHist||{}; const dec={COMPRAR:{n:0,ok:0,sum:0,nr:0,esc:0},MANTENER:{n:0,ok:0,sum:0,nr:0,esc:0},ESPERAR:{n:0,ok:0,sum:0,nr:0,esc:0},VENDER:{n:0,ok:0,sum:0,nr:0,esc:0}};
  ...
  Object.keys(TH).forEach(t=>{ const T=(t||'').toUpperCase(); const a=(DB.analisis||[]).find(x=>(x.ticker||'').toUpperCase()===T)||{}; let ahora=(typeof precioDe==='function')?num(precioDe(a)):num(a.cotizacion); if(!(ahora>0))ahora=num(((DB.valores||{})[T]||{}).precioActual);
...
function renderProxMos(){
  ...
  var rows=(DB.analisis||[]).map(function(a){ var t=(a.ticker||'').toUpperCase(); var cot=(typeof precioDe==='function')?num(precioDe(a)):num(a.cotizacion); var mn=num(a.poMin),mx=num(a.poMax); var po=(typeof poBaseDe==='function')?num(poBaseDe(a)):((mn&&mx)?(mn+mx)/2:(num(a.precioObjetivo)||mx||mn||0));   /* [A6] */
...
function renderPanelDash(){
  ...
  // NOTA: esta es la ÚNICA fuente del score (se calcula una vez por render de renderPanelDash).
  // El KPI del banner de arriba (#panelHeroKpi) se rellena AQUÍ MISMO con el mismo `H`, para que
  // nunca pueda divergir del número que se ve en esta sección — antes se calculaba también en
  // renderPanel() (02-hogar.js) de forma independiente, y como renderPanelDash() se re-ejecuta más
  // tarde tras cargar cotizaciones/fundamentales async, los dos números podían no coincidir.
  if(typeof saludFinanciera==='function'){ try{ const H=saludFinanciera();
    const _phk=document.getElementById('panelHeroKpi');
    if(_phk){ if(H.score!=null){ const sc0=Math.round(H.score); const col0=sc0>=70?'#86efac':(sc0>=50?'#fde68a':'#fca5a5'); _phk.innerHTML=`<div class="big" style="color:${col0}">${sc0}<span>/100</span></div><div class="cap">Salud financiera</div>`; } else { _phk.innerHTML='<div class="big">—</div><div class="cap">Salud financiera</div>'; }
  ...
  // Oportunidades
  const ana=(DB.analisis||[]).map(a=>{const cot=(typeof precioDe==='function')?num(precioDe(a)):num(a.cotizacion),ent=num(a.precioEntrada),obj=(typeof poBaseDe==='function')?num(poBaseDe(a)):num(a.precioObjetivo);   /* [A6] */return {t:a.ticker,pot:(cot&&obj)?(obj-cot)/cot:null,barata:!!(cot&&ent&&cot<=ent*1.05)};});
...
function renderPlanLote(){
  ...
  /* Celda por año = COMPRAS REALES de ese año (execBuyEur, suma de operaciones). Si un año no tiene
     compra, casilla vacía editable para fijar un override manual (la prorrata vive en el panel de abajo). */
  ...
  /* ===== RENDER ESCRITORIO v3 — estilo Desglose: grupos plegables por tipo + fila Plan/Real ===== */
  const GORDER=[['joya','👑 Joyas'],['nucleo','⬢ Núcleo'],['mantener','● Mantener'],['','◦ Sin clasificar']];
  const inGroup=g=>allTk.filter(t=>tipoOf(t)===g).sort((a,b)=>(invByT[b]||0)-(invByT[a]||0));
  window._dvGColl=window._dvGColl||{};
  const nCols=6+yrs.length;
  const dHead='<tr><th>Empresa</th><th class="num">Invertido</th><th class="num">% act</th><th class="num">Objetivo</th><th class="num">A asignar</th>'+yrs.map(y=>`<th class="num yc">${y}</th>`).join('')+'<th class="num tot">Total plan</th></tr>';
  const planYCells=t=>yrs.map(y=>{ const pin=aYear(t,y), isPin=pin>0.5, auto=schedY(t,y); return `<td class="yc"><input type="number" step="100" class="anaInp dv2-in" data-asig="${t}|${y}" value="${isPin?Math.round(pin):''}" placeholder="${auto>0.5?eurK(auto):''}" title="${isPin?('Override manual '+fmt(pin)+' — borra para volver a la prorrata'):('Vacío = prorrata automática'+(auto>0.5?(' ('+eurK(auto)+'€)'):''))}"${isPin?' style="font-weight:700;color:#b45309;background:#fffbeb"':''}></td>`; }).join('');
  const realYCells=t=>yrs.map(y=>{ const inv=execY(t,y), auto=schedY(t,y); if(inv>0.5)return `<td class="num dv2-up" title="Comprado en ${y}: ${fmt(inv)}">${eurK(inv)}</td>`; if(auto>0.5)return '<td class="num dv2-b">pdte</td>'; return '<td class="num dv2-z">·</td>'; }).join('');
  let rows='';
  GORDER.forEach(gp=>{ const g=gp[0], label=gp[1]; const list=inGroup(g); if(!list.length)return;
    const gInv=list.reduce((s,t)=>s+(invByT[t]||0),0), gObj=list.reduce((s,t)=>s+objEur(t),0), gAsig=list.reduce((s,t)=>s+Math.max(0,asignar(t)),0);
  ...
  const blank4='<td class="dv2-z">·</td><td class="dv2-z">·</td><td class="dv2-z">·</td><td class="dv2-z">·</td>';
  let sumRows='<tr class="dv2-amar"><td colspan="'+nCols+'">📊 Resumen por año</td></tr>';
  sumRows+='<tr class="dv2-sum"><td>Presupuesto</td>'+blank4+yrs.map(y=>`<td class="yc"><input type="number" data-lotedisp="${y}" value="${Math.round(presBruto(y))}" class="dv2-in dv2-pres" title="Presupuesto bruto de ${y} (editable)"></td>`).join('')+'<td class="dv2-z">·</td></tr>';
  sumRows+='<tr class="dv2-sum"><td>Disponible</td>'+blank4+yrs.map(y=>`<td class="num">${eurK(dispShown(y))}</td>`).join('')+'<td class="dv2-z">·</td></tr>';
  sumRows+='<tr class="dv2-sum"><td>Reparto asignado</td>'+blank4+yrs.map(y=>`<td class="num">${eurK(autoYear(y))}</td>`).join('')+'<td class="dv2-z">·</td></tr>';
  sumRows+='<tr class="dv2-sum"><td>Sin asignar</td>'+blank4+yrs.map(y=>{ const sa=dispShown(y)-autoYear(y); return `<td class="num ${sa>0.5?'up':(sa<-0.5?'dn':'')}">${sa>0?'+':''}${eurK(sa)}</td>`; }).join('')+'<td class="dv2-z">·</td></tr>';
  const deskHTML='<div class="dv-desk"><input type="range" id="loteScroll" min="0" max="1000" value="0" title="Desliza para moverte por los años" class="dv-slider"><div class="dtable" id="loteScrollBox"><table class="dv2-tbl"><thead>'+dHead+'</thead><tbody>'+rows+sumRows+'</tbody></table></div></div>';
  ...
  const mdetail=(t,i)=>{ const estPill=held.includes(t)?'<span class="pill g">Cartera</span>':'<span class="pill b">Sin comprar</span>';  /* misma etiqueta que la fila de escritorio */
    return `<div class="codet" data-coi="${i}"${i!==selIdx?' style="display:none"':''}>`
      ...
      +`<div class="yrgrid">${yrs.map(y=>{ const inv=execY(t,y), pin=aYear(t,y), isPin=pin>0.5; if(inv>0.5){ return `<label class="yg"><span>${y}</span><div class="num" style="font-weight:800;color:#16a34a">${eurK(inv)}€</div><div class="dlbl" style="color:#16a34a">comprado</div></label>`; } return `<label class="yg"><span>${y}</span><input type="number" step="100" class="anaInp asig" data-asig="${t}|${y}" value="${isPin?Math.round(pin):''}" placeholder=""${isPin?' style="font-weight:700;color:#b45309;background:#fffbeb"':''}></label>`; }).join('')}</div>`
      +`</div>`; };
  ...
  /* Reparto automático colapsable: cerrado se ve sólo hasta los cuadros €/año; el detalle por empresa se despliega al clic */
  if(!el._loteARBound){ el._loteARBound=true; el.addEventListener('click',function(e){ var h=e.target.closest('[data-autorep-tgl]'); if(!h)return; window._autoRepOpen=!window._autoRepOpen; var box=h.parentNode; var det=box.querySelector('.arDetail'); var chev=h.querySelector('.arChev'); if(det)det.style.display=window._autoRepOpen?'block':'none'; if(chev)chev.style.transform='rotate('+(window._autoRepOpen?90:0)+'deg)'; }); }
  /* Grupos de tipo plegables (bandas): clic en la banda oculta/muestra sus filas Plan/Real */
  if(!el._dvBandBound){ el._dvBandBound=true; el.addEventListener('click',function(e){ if(e.target.closest('.dv2-del')||e.target.closest('[data-ficha]'))return; var b=e.target.closest('.dv2-band'); if(!b)return; var g=b.getAttribute('data-dvg'); window._dvGColl=window._dvGColl||{}; window._dvGColl[g]=!window._dvGColl[g]; var coll=window._dvGColl[g]; b.classList.toggle('collapsed',coll); el.querySelectorAll('tr[data-dvg="'+g+'"]').forEach(function(r){ if(r!==b) r.style.display=coll?'none':''; }); }); }
