...
function renderPanelDash(){
  ...
  if(typeof crecimientoAtribucion==='function'){ try{ const A=crecimientoAtribucion(12); if(A&&(Math.abs(A.crecValor)>0.5||Math.abs(A.dividendos)>0.5)){ const sg=x=>(x>=0?'+':'')+fmt(x); const cs=[['Aportación',sg(A.aportacion),'','lo que has metido'],['Mercado',sg(A.revaloriz),A.revaloriz>=0?'pos':'neg','revalorización'],['Dividendos',sg(A.dividendos),'pos','cobrados (a caja)'],['Retorno (mkt+div)',sg(A.retorno),A.retorno>=0?'pos':'neg','últimos '+A.meses+' meses']]; html+=block('De dónde viene tu crecimiento','atribucion',cs); } }catch(e){} }
  // Patrimonio
