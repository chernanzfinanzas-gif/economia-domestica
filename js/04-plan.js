...
function renderProxMos(){
  ...
  var _dsFac=function(ds){ if(!ds||ds.score==null)return 1; var m={'Muy seguro':1,'Seguro':0.9,'Vigilar':0.6,'Frágil':0.3,'Recorte probable':0.1}; var f=(m[ds.banda]!=null)?m[ds.banda]:(ds.score>=80?1:ds.score>=60?0.9:ds.score>=40?0.6:ds.score>=20?0.3:0.1); if(ds.topeDuro&&ds.topeDuro.activo)f=Math.min(f,0.3); return f; };
  ...
  var _razon=function(r){ var q=r.rating?('calidad '+r.rating):'calidad sin calificar'; var dmap={'Muy seguro':'dividendo muy seguro','Seguro':'dividendo seguro','Vigilar':'dividendo a vigilar','Frágil':'dividendo frágil','Recorte probable':'dividendo en riesgo de recorte'}; var dtxt=(r.ds&&r.ds.score!=null)?((dmap[r.ds.banda]||'dividendo '+r.ds.banda)+' ('+r.ds.score+')'):'dividendo sin evaluar'; var ptxt=(r.pot==null)?'sin precio objetivo':(r.pot>=0?('potencial +'+(r.pot*100).toFixed(0)+'% hasta tu PO'):('cotiza '+Math.abs(r.pot*100).toFixed(0)+'% por encima de tu PO')); var enZona=(r.entMax>0&&r.cot<=r.entMax); var concl; if(r.pot!=null&&r.pot<0)concl='cara respecto a tu valoración, no es momento de entrar'; else if(enZona)concl='en tu zona de entrada, candidata a compra'; else if(r.pot!=null&&r.pot>0.15)concl='con recorrido pero aún no en tu banda de entrada'; else concl='cerca de tu valoración, margen ajustado'; var cav=''; if(r.ds&&r.ds.score!=null){ if(r.ds.topeDuro&&r.ds.topeDuro.activo)cav=' No cuentes su dividendo como renta estable.'; else if(r.ds.score<40)cav=' No la compres por la renta.'; else if(r.ds.score<60)cav=' No la compres solo por el dividendo.'; } return 'Empresa de '+q+', '+dtxt+', '+ptxt+'; '+concl+'.'+cav; };
  var _razonRow=function(r){ var d=r.dec?('<b style="color:'+_decCol(r.dec)+'">'+r.dec+'</b> · '):''; return '<tr class="mos-razon"><td></td><td colspan="8" style="font-size:11px;color:#475569;padding:0 10px 9px 10px;line-height:1.45">💬 '+d+_razon(r)+'</td></tr>'; };
  ...
  var rows=(DB.analisis||[]).map(function(a){ var t=(a.ticker||'').toUpperCase(); var cot=num(a.cotizacion); var mn=num(a.poMin),mx=num(a.poMax); var po=(mn&&mx)?(mn+mx)/2:(num(a.precioObjetivo)||mx||mn||0);
    ...
    return {t:t,cot:cot,po:po,pot:pot,rating:rating,cal:cal,rpd:rpd,dec:dec,entMax:entMax,held:!!held[t],ds:ds,sf:sf,score:(pot!=null?Math.max(0,pot):0)*100*(cal/100)*rpd*sf}; }).filter(function(x){ return x.cot>0&&x.po>0; });
