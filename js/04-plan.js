...
function renderPanelDash(){
  ...
  if(typeof carteraRentabilidad==='function'){ try{ const R=carteraRentabilidad(renderPanelDash); if(R){ const pc=x=>x==null?'—':((x>=0?'+':'')+(x*100).toFixed(1)+'%'); const cs=[['TIR (anual, s/dinero)',pc(R.xirr),R.xirr>=0?'pos':'neg','ponderada por dinero'],['Rentab. temporal (anual)',pc(R.twrAnual),R.twrAnual>=0?'pos':'neg','comparable al índice']]; if(R.hayIbex){ cs.push(['Alfa vs IBEX-35',pc(R.alfa),(R.alfa!=null&&R.alfa>=0)?'pos':'neg','TWR − IBEX (precio)']); cs.push(['Ventaja vs IBEX',(R.valDif>=0?'+':'')+fmt(R.valDif),R.valDif>=0?'pos':'neg','mismo dinero al índice']); } html+=block('Rentabilidad','ranking',cs); } }catch(e){} }
  // Atribución del crecimiento (aportación / mercado / dividendos)
  if(typeof crecimientoAtribucion==='function'){ try{ const A=crecimientoAtribucion(12); if(A&&Math.abs(A.total)>0.5){ const sg=x=>(x>=0?'+':'')+fmt(x); const cs=[['Aportación',sg(A.aportacion),'','lo que has metido'],['Mercado',sg(A.revaloriz),A.revaloriz>=0?'pos':'neg','revalorización'],['Dividendos',sg(A.dividendos),'pos','cobrados'],['Crecimiento total',sg(A.total),A.total>=0?'pos':'neg','últimos '+A.meses+' meses']]; html+=block('De dónde viene tu crecimiento','atribucion',cs); } }catch(e){} }
  // Patrimonio
