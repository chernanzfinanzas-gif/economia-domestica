...
function renderProxMos(){
  ...
  var _foBadge=function(fo){ if(!fo)return '<span class="muted" title="Análisis forense no disponible en este dossier" style="cursor:help;font-size:11px">—</span>'; if(fo.aplica===false)return '<span class="muted" title="Forense: no aplica ('+((fo.motivo||'rama financiera')+'').replace(/"/g,'&quot;')+')" style="cursor:help;font-size:11px">n/a</span>'; if(!fo.aplica)return '<span class="muted" style="font-size:11px">—</span>'; var scores='Piotroski '+(fo.piotroski?fo.piotroski.score:'-')+'/9 · Altman '+(fo.altman?fo.altman.z:'-')+' ('+(fo.altman?fo.altman.zona:'-')+') · Beneish '+(fo.beneish?fo.beneish.m:'-')+((fo.sloan&&fo.sloan.accruals!=null)?' · Sloan '+fo.sloan.accruals+'%':''); var has=fo.flags&&fo.flags.length; if(!has)return '<span title="'+('Sin alertas forenses · '+scores).replace(/"/g,'&quot;')+'" style="cursor:help;background:#16a34a;color:#fff;border-radius:6px;padding:1px 7px;font-size:10.5px;font-weight:700">✓</span>'; var v=_foVeto(fo); var tip=((v?'VETO forense — ':'Alerta forense — ')+fo.flags.join(' · ')+' · '+scores).replace(/"/g,'&quot;'); return '<span title="'+tip+'" style="cursor:help;background:'+(v?'#991b1b':'#dc2626')+';color:#fff;border-radius:6px;padding:1px 7px;font-size:10.5px;font-weight:700">'+(v?'⚠️ VETO':'⚠️')+'</span>'; };
  var _foTd=function(r){ return '<td style="text-align:center">'+_foBadge(r.fo)+'</td>'; };
  var _foMob=function(fo){ var lab,col; if(!fo){lab='—';col='#94a3b8';} else if(fo.aplica===false){lab='n/a';col='#94a3b8';} else if(!fo.aplica){lab='—';col='#94a3b8';} else if(!(fo.flags&&fo.flags.length)){lab='✓ limpio';col='#16a34a';} else {var v=_foVeto(fo); lab=v?'⚠️ VETO':'⚠️ alerta'; col=v?'#991b1b':'#dc2626';} return '<div class="m"><span>Forense</span><b style="color:'+col+'">'+lab+'</b></div>'; };
  var _razonRow=function(r){ var d=r.dec?('<b style="color:'+_decCol(r.dec)+'">'+r.dec+'</b> · '):''; return '<tr class="mos-razon"><td></td><td colspan="9" style="text-align:left;white-space:normal;word-break:normal;font-size:11px;color:#475569;padding:2px 10px 10px 10px;line-height:1.5">💬 '+d+_razon(r)+_foRazon(r)+'</td></tr>'; };
  ...
  var rows=(DB.analisis||[]).map(function(a){ var t=(a.ticker||'').toUpperCase(); var cot=num(a.cotizacion); var mn=num(a.poMin),mx=num(a.poMax); var po=(mn&&mx)?(mn+mx)/2:(num(a.precioObjetivo)||mx||mn||0);
    ...
    return {t:t,cot:cot,po:po,pot:pot,rating:rating,cal:cal,rpd:rpd,dec:dec,entMax:entMax,held:!!held[t],ds:ds,sf:sf,fo:fo,ff:ff,score:(pot!=null?Math.max(0,pot):0)*100*(cal/100)*rpd*sf*ff}; }).filter(function(x){ return x.cot>0&&x.po>0; });
  ...
  var inner='<div class="mos-formula"><b class="h">¿Qué ordena esto?</b>Junta en un número las <b>tres cosas que buscas al comprar</b>: comprar <b>barato</b> (lejos de tu precio objetivo), que sea <b>buena empresa</b> (calidad del dossier) y que <b>pague dividendo</b>.<br><span class="eq">Índice = potencial a PO × calidad × RPD × seguridad del dividendo</span><br><span class="muted">Cuanto más alto, más "chollo" según tu método. No es una orden de compra: la decisión final la tomas tú mirando la tesis.</span></div>'
    +'<div class="mos-desk"><table><thead><tr><th class="rk">#</th><th class="l">Empresa</th><th>Cotización</th><th>PO base</th><th>Potencial</th><th style="text-align:center">Calidad</th><th>RPD</th><th style="text-align:center" title="Dividend Safety Score — factor del índice">Seguridad</th><th style="text-align:center" title="Capa forense (Piotroski / Altman / Beneish / Sloan) de las ya analizadas: ✓ sin alertas · ⚠️ alerta · VETO fraude/insolvencia">Forense</th><th>Índice</th></tr></thead><tbody>'+trs+'</tbody></table></div>'
    +'<div class="mos-mob">'+cards+'</div>'
    +'<div class="mos-foot"><b>Potencial a PO</b> = cuánto puede subir hasta tu precio objetivo (media de PO pesimista/optimista). <b>Calidad</b> = rating del método (AAA→C). <b>RPD</b> = dividendo/cotización. <b>Seguridad</b> = Dividend Safety Score; multiplica el índice por un factor (Muy seguro ×1 · Seguro ×0,9 · Vigilar ×0,6 · Frágil ×0,3 · Recorte/tope duro ×0,1–0,3) para hundir las trampas de dividendo (RPD alta pero renta poco fiable). <b>Forense</b> = contraste automático de las cuentas (Piotroski · Altman · Beneish · Sloan): <span style="color:#16a34a;font-weight:700">✓</span> sin alertas, <span style="color:#dc2626;font-weight:700">⚠️</span> alerta, <span style="color:#991b1b;font-weight:700">VETO</span> indicio de fraude/insolvencia (pasa el ratón por encima para el detalle); «—» si el dossier no trae capa forense y «n/a» si no aplica (rama financiera). Es informativo, no altera el índice. Las empresas <b>por encima de su PO</b> salen en gris al final: caras según tu tesis. «🟢 en zona» = cotiza por debajo de tu banda de entrada. Solo salen las que tienen precio objetivo en Análisis.</div>';
...
// === Avisos de la capa forense (Piotroski / Altman / Beneish / Sloan) para el Centro de Alertas ===
function _foAvVeto(fo){ return !!fo&&fo.aplica&&((fo.veto===true)||(fo.beneish&&(''+fo.beneish.senal).indexOf('manipulaci')>=0)||(fo.altman&&fo.altman.zona==='riesgo')); }
function forenseAlertas(){
  var out=[]; var held=(typeof heldTickerSet==='function')?heldTickerSet():new Set();
  (DB.analisis||[]).forEach(function(a){
    var t=(a.ticker||'').toUpperCase();
    var fo=a.forense||((typeof _tesisCache!=='undefined'&&_tesisCache&&_tesisCache[t])?_tesisCache[t].forense:null)||null;
    if(!fo||!fo.aplica||!fo.flags||!fo.flags.length)return;
    var v=_foAvVeto(fo); var enCart=held.has&&held.has(t);
    out.push({pri:v?1:3, cls:v?'r':'a', goto:'analisis', tick:t, tipo:'tesis',
      txt:(v?'🔒 ':'🔬 ')+'<b>'+t+'</b> — '+(v?'VETO forense: ':'alerta forense: ')+fo.flags.join(' · ')+(v?'. No procede COMPRAR en firme hasta investigarlo':'')+(enCart?' · en cartera':'')});
  });
  out.sort(function(a,b){ return a.pri-b.pri || (''+a.tick).localeCompare(''+b.tick); });
  return out;
}
function renderPanelDash(){
  ...
  try{ if(typeof forenseAlertas==='function') forenseAlertas().forEach(x=>avisos.push(x)); }catch(e){}
