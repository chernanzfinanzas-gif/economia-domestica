...
function renderPlanLote(){
  ...
  /* Anotación por año: invertido REAL de ese año (derivado de operaciones; suma varias compras). */
  const yrAnnot=(t,y)=>{ const inv=execY(t,y); return inv>0.5 ? '<div class="qbadge ok" title="Invertido real en '+y+': '+fmt(inv)+' (suma de tus operaciones)">✓ '+eurK(inv)+'€</div>' : ''; };
  /* Celda por año: la prorrata sale como placeholder gris (editable → override manual, que sí se
     guarda en DB.planCompras). Si hay override, se muestra en ámbar. Debajo, el invertido real. */
  const yrCells=t=>yrs.map(y=>{ const pin=aYear(t,y), auto=schedY(t,y), isPin=pin>0.5;
    return `<td class="yc"><input type="number" step="100" class="anaInp asig" data-asig="${t}|${y}" value="${isPin?Math.round(pin):''}" placeholder="${auto>0.5?Math.round(auto):''}" title="${isPin?('Override manual: '+fmt(pin)+' (borra para volver a la prorrata)'):(auto>0.5?('Prorrata: '+fmt(auto)+' — escribe para fijar este año'):'sin plan este año')}"${isPin?' style="font-weight:700;color:#b45309;background:#fffbeb"':''}>${yrAnnot(t,y)}</td>`; }).join('');
  ...
  const mdetail=(t,i)=>{ const estPill=held.includes(t)?'<span class="pill g">Cartera</span>':'<span class="pill b">Nueva</span>';
    return `<div class="codet" data-coi="${i}"${i!==selIdx?' style="display:none"':''}>`
      ...
      +`<div class="yrgrid">${yrs.map(y=>{ const pin=aYear(t,y), auto=schedY(t,y), isPin=pin>0.5; return `<label class="yg"><span>${y}</span><input type="number" step="100" class="anaInp asig" data-asig="${t}|${y}" value="${isPin?Math.round(pin):''}" placeholder="${auto>0.5?Math.round(auto):''}"${isPin?' style="font-weight:700;color:#b45309;background:#fffbeb"':''}>${yrAnnot(t,y)}</label>`; }).join('')}</div>`
      +`</div>`; };
