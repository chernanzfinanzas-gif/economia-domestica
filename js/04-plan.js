...
function renderPanelDash(){
  ...
  _heldSet.forEach(t=>{ const m=(DB.monitor||{})[t]||{}; if(!m.informe)return; ['Q1','Q2','Q3','Q4'].forEach(qc=>{ const key=nowY+'-'+qc; const done=!!(m.rev&&m.rev[key]); if((typeof qPassed==='function'&&qPassed(t,qc,nowY))&&!done) avisos.push({pri:2,cls:'a',goto:'monitor',sig:'S5',tick:t,txt:`📊 <b>${t}</b> — ${qc} ${nowY} publicado sin revisar`}); }); });
