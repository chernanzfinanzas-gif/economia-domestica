...
function renderPanelDash(){
  ...
  (DB.analisis||[]).forEach(a=>{ const c=num(a.cotizacion),pMin=num(a.poMin),pMax=num(a.poMax); const pMed=(pMin&&pMax)?(pMin+pMax)/2:(pMax||pMin||0); if(c<=0)return; const t=(a.ticker||'').toUpperCase(); const p=_heldP[t];
    if(pMax>0&&c>=pMax){ avisos.push({pri:1,cls:'a',goto:'analisis',txt:`🎯 <b>${t}</b> — ha alcanzado tu precio objetivo máximo (${fmt(c)} ≥ PO ${fmt(pMax)})${p?` · tienes ${p.acciones} acc., ¿recoger beneficios?`:' · sobrevalorada, no comprar'}`}); }
    else if(pMed>0&&c>=pMed){ avisos.push({pri:3,cls:'a',goto:'analisis',txt:`🎯 <b>${t}</b> — en tu PO base (${fmt(c)} ≥ ${fmt(pMed)}), revisa la tesis${p?' · en cartera':''}`}); } });
