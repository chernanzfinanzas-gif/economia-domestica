...
/* [C16 · 27-jul-2026] Tercera unidad. Hasta ahora el eje solo sabía euros o porcentajes, y la
   gráfica de consumo de Mazinger (L/100km) salía rotulada en € y redondeada a entero: con consumos
   entre 4,8 y 7,1 el eje ponía «0€ · 2€ · 4€ · 5€ · 7€». `unit` es opcional y sin ella el
   comportamiento es exactamente el de antes, así que las otras gráficas no se enteran. */
function gUnitLbl(v,unit){ return (Math.round(v*10)/10).toString().replace('.',',')+(unit?' '+unit:''); }
function gYAxis(mn,mx,padL,padT,plotH,W,pct,unit){ const N=4; let g=''; for(let i=0;i<=N;i++){ const v=mn+(mx-mn)*i/N; const y=padT+plotH-((mx-mn)?((v-mn)/(mx-mn)):0)*plotH; const lbl=unit?gUnitLbl(v,unit):(pct?(v.toFixed(0)+'%'):gAbrev(v)); g+=`<line x1="${padL}" y1="${y.toFixed(1)}" x2="${W-12}" y2="${y.toFixed(1)}" stroke="#eef2f7"/><text x="${(padL-4).toFixed(1)}" y="${(y+3).toFixed(1)}" text-anchor="end" font-size="11" fill="#64748b">${lbl}</text>`; } return g; }
function gEsc(x){ return (''+x).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
function gBars(title,labels,series,opt){ opt=opt||{}; const W=Math.max(340,labels.length*(series.length>1?40:30)+50),H=num(opt.h)||280,padL=46,padB=26,padT=10;
...
function _gtBind(){ if(_gtBound)return; _gtBound=true;
  document.addEventListener('mousemove',e=>{
    ...
    if(tip){ let h=`<div style="font-weight:700;margin-bottom:2px">${gEsc(D.labels[bi])}</div>`;
      D.series.forEach(s=>{ const v=num(s.vals[bi]); const nm=s.name?`${gEsc(s.name)}: `:''; h+=`<div><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:${s.color};margin-right:5px"></span>${nm}<b>${D.unit?gUnitLbl(v,D.unit):(D.pct?v.toFixed(1)+'%':fmt(v))}</b></div>`; });
...
function _gtRegister(labels,series,geom){
  ...
  _gtReg[id]={labels,series,xs,W:geom.W,Y,pct:!!geom.pct,unit:geom.unit||''};
  _gtBind();
...
/* [09-sep-2026] `comision` se añade a la reconstrucción de las cerradas: de aquí come el FIFO de
   Fiscalidad (04-plan.js), y sin el campo una posición archivada tributaría por una ganancia mayor
   de la real. Solo se copia si venía: ausente sigue significando «no registrada». */
function _allOps(){ const out=(DB.operaciones||[]).slice(); (DB.cerradas||[]).forEach(c=>{ const t=(c.ticker||'').toUpperCase(); (c.ops||[]).forEach(o=>{ const _o={ticker:t,fecha:o.fecha,tipo:o.tipo,acciones:o.acciones,precio:o.precio}; if(o.comision!=null&&o.comision!=='') _o.comision=o.comision; out.push(_o); }); }); return out; }
...
function renderGraficas(){ const el=$('#grafBody'); if(!el)return; const kp=$('#grafKpis');
  ...
  { const snaps=(typeof patSnaps==='function'?patSnaps():[]); const labels=snaps.map(s=>(s.fecha||'').slice(0,7));
    ...
    window._grafWf=window._grafWf||'all';
    let cCascada='';
    { const _dw=(typeof carteraEvolData==='function')?carteraEvolData(renderGraficas):null;
      if(_dw&&_dw.ok){
        const _wy=[...new Set(_dw.labels.map(l=>+l.split('-')[0]))].sort((a,b)=>b-a);
        if(window._grafWf!=='all' && !_wy.includes(+window._grafWf)) window._grafWf='all';
        let _A,_tit;
        if(window._grafWf==='all'){ _A=_atribFrom(_evoState(_dw,null),_evoState(_dw,_dw.labels.length-1)); _tit='Todo el periodo'; }
        else { const _Y=+window._grafWf; _A=(typeof atribucionRango==='function')?atribucionRango(_Y*100+1,_Y*100+12):null; _tit='Año '+_Y; }
        const _selHTML='<div class="toolbar" style="margin:2px 0 8px;gap:8px;align-items:center"><span class="muted">Periodo:</span> <select id="grafWfSel" class="anaInp" style="width:150px"><option value="all"'+(window._grafWf==='all'?' selected':'')+'>Todo el periodo</option>'+_wy.map(y=>'<option value="'+y+'"'+(+window._grafWf===y?' selected':'')+'>'+y+'</option>').join('')+'</select></div>';
        if(_A){ const _efc=_A.aportacion-_A.dividendos, _vi=num(_A.valIni), _vf=num(_A.valFin);
          const _wf=_atribWaterfall([{label:'Inicio',base:_vi},{label:'Dividendo',delta:_A.dividendos,col:'#16a34a'},{label:'Efectivo nuevo',delta:_efc,col:'#2563eb'},{label:'Mercado',delta:_A.revaloriz,col:'#0d9488'},{label:'Valor final',base:_vf}]).replace('max-width:700px;display:block','max-width:960px;display:block;margin-left:auto;margin-right:auto');
          cCascada='<div class="card" style="margin:0"><div style="font-weight:700;font-size:13px">De dónde viene tu patrimonio · '+_tit+'</div><div class="muted" style="font-size:11.5px;margin-bottom:2px">'+fmt(_vi)+' → '+fmt(_vf)+' · efectivo nuevo + dividendos + revalorización del mercado</div>'+_selHTML+_wf+'</div>';
        } else { cCascada='<div class="card" style="margin:0">'+_selHTML+'<div class="muted" style="font-size:12px">Sin datos para ese periodo.</div></div>'; }
      } else { cCascada='<div class="card" style="margin:0"><div style="font-weight:700;font-size:13px;margin-bottom:4px">De dónde viene tu patrimonio</div><div class="muted" style="font-size:12px">Cargando cotizaciones del repo… (necesita conexión)</div></div>'; }
    }
    const pos=(typeof invPositions==='function'?invPositions():[]).filter(p=>p.acciones>0.0001);
    ...
    const _nyB=new Date().getFullYear(); const _snowEnd=Math.max(2034,_nyB); let _accSnow=0; const _bnY=[],_snowReal=[],_snowProj=[]; for(let y=2011;y<=_snowEnd;y++){ _bnY.push(String(y)); if(y<=_nyB){ _accSnow+=(typeof calBrutoCarteraAnio==='function'?num(calBrutoCarteraAnio(y)):(typeof simYearTotal==='function'?num(simYearTotal(y)):0)); _snowReal.push(_accSnow); _snowProj.push(0); } else { _accSnow+=(typeof simYearTotal==='function'?num(simYearTotal(y)):0); _snowReal.push(0); _snowProj.push(_accSnow); } } const _bolaW=Math.max(340,_bnY.length*40+50); const cBola=gBars('Bola de nieve del dividendo · real cobrado + proyección Simulador a '+_snowEnd,_bnY,[{name:'Real cobrado',color:'#16a34a',vals:_snowReal},{name:'Proyección (Simulador)',color:'#f59e0b',vals:_snowProj}],{h:Math.round(_bolaW/1.5)});
    ...
    const _nyY=new Date().getFullYear(); const _yy=[],_yv=[]; const _dpa=DB.divPorAccion||{}; /* [A5] serie histórica desde la fuente única; _dpa queda solo de respaldo */
    const _dpaY=(t,y)=>{ const v=(typeof dpaAnual==='function')?dpaAnual(t,y,{soloReal:true}):null; return (v!=null)?v:num((_dpa[t]||{})[y]||0); }; const _allT=[...new Set((DB.operaciones||[]).map(o=>(o.ticker||'').toUpperCase()).filter(Boolean))]; for(let y=2011;y<=_nyY;y++){ let coste=0; (DB.operaciones||[]).forEach(o=>{ const oy=+((o.fecha||'').slice(0,4)); if(oy&&oy<=y)coste+=(o.tipo==='venta'?-1:1)*num(o.acciones)*num(o.precio); }); let divA=0; _allT.forEach(t=>{ const sh=(typeof realSharesAt==='function')?realSharesAt(t,y):0; divA+=sh*num(_dpaY(t,y)); }); if(coste>0){ _yy.push(String(y)); _yv.push(divA/coste*100); } } const cYoC=_yy.length?gLine('Yield on cost por año (%)',_yy,_yv,{pct:true,color:'#16a34a'}):'<div class="card" style="margin:0"><div style="font-weight:700;font-size:13px;margin-bottom:4px">YoC por año</div><div class="muted" style="font-size:12px">Sin datos.</div></div>'; const _rpY=[],_rpV=[]; for(let y=2011;y<=_nyY;y++){ let val=0,divA=0; _allT.forEach(t=>{ const sh=(typeof realSharesAt==='function')?realSharesAt(t,y):0; if(sh>0.0001){ divA+=sh*num(_dpaY(t,y)); val+=sh*priceAtFB(t,Date.UTC(y,11,31)); } }); if(val>0){ _rpY.push(String(y)); _rpV.push(divA/val*100); } } const cRPD=_rpY.length?gLine('RPD por año (%) · dividendo \u00f7 cotización',_rpY,_rpV,{pct:true,color:'#0ea5e9'}):'<div class="card" style="margin:0"><div style="font-weight:700;font-size:13px;margin-bottom:4px">RPD por año</div><div class="muted" style="font-size:12px">Sin datos de cotización aún.</div></div>';
    ...
    const _dpsNow=(t)=>num((typeof dpaAnual==='function')?dpaAnual(t,nowY):null);
    ...
    let cDivAnual='';
    { const _da=new Date().getFullYear(); const _daEnd=Math.max(2034,_da); const _daY=[],_daReal=[],_daProj=[];
      for(let y=2011;y<=_daEnd;y++){ _daY.push(String(y));
        if(y<=_da){ _daReal.push(typeof calBrutoCarteraAnio==='function'?num(calBrutoCarteraAnio(y)):(typeof simYearTotal==='function'?num(simYearTotal(y)):0)); _daProj.push(0); }
        else { _daReal.push(0); _daProj.push(typeof simYearTotal==='function'?num(simYearTotal(y)):0); } }
      const _daW=Math.max(340,_daY.length*40+50);
      cDivAnual=gBars('Dividendo bruto por año · real cobrado + previsión Simulador a '+_daEnd,_daY,[{name:'Real cobrado',color:'#16a34a',vals:_daReal},{name:'Previsión (Simulador)',color:'#f59e0b',vals:_daProj}],{h:Math.round(_daW/1.5)}); }
    ...
    _carteraInner=`<div class="graf-big">${cAport}</div><div class="graf-big">${cCascada}</div><div class="graf-big">${c4}</div><div class="graf-grid">${c5}${c5b}${cRent}${cObj}</div>`;
    _divInner=`<div class="graf-grid">${c7}${cBola}${cDivAnual}${cYoC}${cRPD}${cDivEmp}</div>`; }
  ...
  el.innerHTML=BB('hogar','🏠','Hogar','gasto · ahorro',_hogarInner)
    +BB('cartera','💼','Cartera y patrimonio','valor · rentabilidad',_carteraInner)
    +BB('div','💰','Dividendos e ingresos pasivos','renta · bola de nieve',_divInner);
...
document.addEventListener('change',e=>{ if(e.target&&e.target.id==='grafWfSel'){ window._grafWf=e.target.value; if(typeof renderGraficas==='function')renderGraficas(); } });
...
/* [C8 · 27-jul-2026] INACTIVA: su contenedor #presRenov no existe en el HTML, así que salía en
   la primera línea. Ya no se llama desde renderAllFull. Se conserva porque el aviso de
   renovaciones del Panel sí usa renovList(); si el bloque vuelve al HTML, esto vuelve a valer. */
