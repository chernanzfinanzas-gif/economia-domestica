...
function renderRentabEmpresas(){ const el=$('#rentaBody'); if(!el)return; const kp=$('#rentaKpis'); const R=rentabilidadEmpresas(renderRentabEmpresas);
  ...
  if(kp)kp.innerHTML=[['TIR cartera (anual)',CR?pc(CR.xirr):'—',(CR&&CR.xirr>=0)?'pos':'neg'],['Alfa vs IBEX (anual)',(CR&&CR.alfa!=null)?pc(CR.alfa):'—',(CR&&CR.alfa!=null&&CR.alfa>=0)?'pos':'neg'],['Ventaja vs IBEX (€)',(CR&&CR.valDif!=null)?sgv(CR.valDif):'—',(CR&&CR.valDif>=0)?'pos':'neg'],['Valor cartera',fmt(R.totVal),'']].map(k=>`<div class="card"><div class="lbl">${k[0]}</div><div class="val ${k[2]||''}">${k[1]}</div></div>`).join('');
  ...
  el.innerHTML=`<div class="sub" style="margin-bottom:8px"><b>Rent. total</b> y <b>TIR</b> miden TU posición (con tus fechas de compra y tus dividendos; TIR = anual ponderada por dinero). <b>TR YTD/1A/3A</b> miden el <b>valor</b> del activo (cotización + dividendos/acción del periodo), para comparar cómo va cada empresa independientemente de cuándo entraste. TR 3A es acumulado. Usa las cotizaciones del repo. La <b>Ventaja vs IBEX (€)</b> compara tu cartera con haber puesto el mismo dinero en el IBEX-35 en <b>total-return</b> (precio + dividendos estimados <input type="number" step="0.1" min="0" value="${(_ibexYield()*100).toFixed(1)}" onchange="setIbexYield(this.value)" style="width:52px;padding:1px 4px;border:1px solid #cbd5e1;border-radius:4px;font-size:11px" title="Yield de dividendo del IBEX asumido para el total-return"> %/año (editable).</div><div style="overflow:auto"><table style="font-size:12px"><thead><tr><th>Empresa</th><th class="num">Peso</th><th class="num">Valor</th><th class="num">Plusvalía</th><th class="num">%</th><th class="num">Div. cobrado</th><th class="num">Rent. total</th><th class="num">TIR</th><th class="num">TR YTD</th><th class="num">TR 1A</th><th class="num">TR 3A</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
...
function renderGraficas(){ const elH=$('#grafHogar'),elI=$('#grafInv'); if(!elH&&!elI)return;
  ...
  if(elH){ const isAll=Y==='all'; const yTag=isAll?'Todos':Y; const movs=isAll?(DB.movimientos||[]).filter(m=>m.fecha):(DB.movimientos||[]).filter(m=>m.fecha&&+m.fecha.slice(0,4)===Y); const gasMes=new Array(12).fill(0),ingMes=new Array(12).fill(0);
  ...
  if(elI){ const snaps=(typeof patSnaps==='function'?patSnaps():[]); const labels=snaps.map(s=>(s.fecha||'').slice(0,7));
    ...
    let cCob='';
    if(typeof coberturaDivGastos==='function'){ const F=coberturaDivGastos(); if(F&&F.cobertura!=null&&F.years&&F.years.length){ cCob=gLines('Dividendo bruto previsto vs gasto anual'+(F.anioIndep?' · independencia en '+F.anioIndep:' · no alcanzado'),F.years.map(String),[{name:'Dividendo bruto',color:'#16a34a',vals:F.divSerie},{name:'Gasto anual',color:'#dc2626',vals:F.gastoSerie}],{h:230}); } else cCob='<div class="card" style="margin:0"><div style="font-weight:700;font-size:13px;margin-bottom:4px">Independencia (dividendos vs gastos)</div><div class="muted" style="font-size:12px">Sin gasto registrado o sin dividendos previstos.</div></div>'; }
    elI.innerHTML=`<div style="margin-bottom:12px">${c4}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px;margin-bottom:12px">${c5}${c5b}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px;margin-bottom:12px">${c6}${c7}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px">${cCap}${cRent}${cBola}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:12px;margin-top:12px">${cObj}${cYoC}${cRPD}</div><div style="margin-top:12px">${cAport}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:12px;margin-top:12px">${cCob}${cDivEmp}</div>`; }
}
