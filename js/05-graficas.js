...
function renderGraficas(){ const el=$('#grafBody'); if(!el)return; const kp=$('#grafKpis');
  ...
  { const snaps=(typeof patSnaps==='function'?patSnaps():[]); const labels=snaps.map(s=>(s.fecha||'').slice(0,7));
    ...
    { const _dw=(typeof carteraEvolData==='function')?carteraEvolData(renderGraficas):null;
      if(_dw&&_dw.ok){
        ...
        if(_A){ const _efc=_A.aportacion-_A.dividendos, _vi=num(_A.valIni), _vf=num(_A.valFin);
          const _wf=_atribWaterfall([{label:'Inicio',base:_vi},{label:'Dividendo',delta:_A.dividendos,col:'#16a34a'},{label:'Efectivo nuevo',delta:_efc,col:'#2563eb'},{label:'Mercado',delta:_A.revaloriz,col:'#0d9488'},{label:'Valor final',base:_vf}]);
    ...
    _carteraInner=`<div class="graf-big">${cAport}</div><div class="graf-big">${cCascada}</div><div class="graf-big">${c4}</div><div class="graf-grid">${c5}${c5b}${cCap}${cRent}${cObj}</div>`;
    _divInner=`<div class="graf-grid">${c7}${cBola}${cYoC}${cRPD}${cDivEmp}${cCob}</div>`; }
