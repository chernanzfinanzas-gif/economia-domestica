...
function renderGraficas(){ const el=$('#grafBody'); if(!el)return; const kp=$('#grafKpis');
  ...
  { const snaps=(typeof patSnaps==='function'?patSnaps():[]); const labels=snaps.map(s=>(s.fecha||'').slice(0,7));
    ...
    const _pctInv=labels.map((_,i)=>{ const _t=ef[i]+inv[i]; return _t>0?inv[i]/_t*100:0; }); const cPeso=labels.length?gLine('Peso invertido sobre el patrimonio (%)',labels,_pctInv,{pct:true,color:'#0d9488'}):'<div class="card" style="margin:0"><div style="font-weight:700;font-size:13px;margin-bottom:4px">Peso invertido sobre el patrimonio</div><div class="muted" style="font-size:12px">Sin snapshots de Patrimonio.</div></div>';
    const pos=(typeof invPositions==='function'?invPositions():[]).filter(p=>p.acciones>0.0001);
    ...
    _carteraInner=`<div class="graf-big">${cAport}</div><div class="graf-big">${c4}</div><div class="graf-grid">${c5}${c5b}${cPeso}${cCap}${cRent}${cObj}</div>`;
