...
function _ibexYield(){ return (typeof DB!=='undefined'&&DB.config&&DB.config.ibexYield!=null)?num(DB.config.ibexYield):0.04; }
...
function carteraEvolData(reRender){ const _ops2=_allOps().filter(o=>o.fecha).sort((a,b)=>(a.fecha||'').localeCompare(b.fecha||''));
  ...
  while(yy<now.getFullYear()||(yy===now.getFullYear()&&mn2<=now.getMonth())){ const cut=Date.UTC(yy,mn2+1,0); const sh={}; let ap=0;
    ...
    labels.push(String(yy).slice(2)+'/'+String(mn2+1).padStart(2,'0')); aport.push(ap); valor.push(val); valdiv.push(val+divAc); if(hayIbex){ const ipx=priceRepoAt('IBEX',cut); const dAp=ap-_prevAp; if(ipx>0&&dAp!==0)ibexUnits+=dAp/ipx; if(ibexUnits>0)ibexUnits*=(1+_ibexYield()/12); ibexVal.push(ipx>0?ibexUnits*ipx:(ibexVal.length?ibexVal[ibexVal.length-1]:0)); } _prevAp=ap; mn2++; if(mn2>11){mn2=0;yy++;} }
