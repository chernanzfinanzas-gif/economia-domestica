...
function renderGraficas(){ const elH=$('#grafHogar'),elI=$('#grafInv'); if(!elH&&!elI)return;
  ...
  if(elI){ const snaps=(typeof patSnaps==='function'?patSnaps():[]); const labels=snaps.map(s=>(s.fecha||'').slice(0,7));
    ...
    elI.innerHTML=`<div style="margin-bottom:12px">${c4}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px;margin-bottom:12px">${c5}${c5b}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px;margin-bottom:12px">${c6}${c7}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px">${cCap}${cRent}${cBola}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:12px;margin-top:12px">${cObj}${cYoC}${cRPD}</div><div style="margin-top:12px">${cAport}</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(360px,1fr));gap:12px;margin-top:12px"><div>${cCob}</div><div>${cDivEmp}</div></div>`; }
}
