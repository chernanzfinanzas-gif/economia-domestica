...
function carteraEvolData(reRender){ const _ops2=_allOps().filter(o=>o.fecha).sort((a,b)=>(a.fecha||'').localeCompare(b.fecha||''));
  ...
  const divEv=[]; const _closT=new Set((DB.cerradas||[]).map(c=>(c.ticker||'').toUpperCase())); const _dvO=DB.dividendos||{}; Object.keys(_dvO).forEach(t=>{ const T=(t||'').toUpperCase(); if(_closT.has(T))return; /* cerradas mandan: evita doble conteo (p. ej. TEF) */ (_dvO[t]||[]).forEach(d=>{ if(d.fecha){ const dm=Date.parse(d.fecha+'T00:00:00'); if(!isNaN(dm))divEv.push({ms:dm,eur:sharesAt(T,dm)*num(d.importe)}); } }); });
...
function carteraRentabilidad(reRender){ const d=carteraEvolData(reRender); if(!d.ok)return null;
  ...
  const _closT=new Set((DB.cerradas||[]).map(c=>(c.ticker||'').toUpperCase())); const _dvO=DB.dividendos||{}; Object.keys(_dvO).forEach(t=>{ const T=(t||'').toUpperCase(); if(_closT.has(T))return; /* cerradas mandan: evita doble conteo */ (_dvO[t]||[]).forEach(dd=>{ if(dd.fecha){ const dm=Date.parse(dd.fecha+'T00:00:00'); if(!isNaN(dm)){ const e=shAt(T,dm)*num(dd.importe); if(e)cf.push({t:dm,a:e}); } } }); });
