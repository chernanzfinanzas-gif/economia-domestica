...
function renderInvClosed(){
  ...
  el.innerHTML=`<div class="pos-blk${op}" data-invclosedblk="1" style="margin-top:12px"><div class="pos-blk-h"><span class="arw">▶</span><span class="bt">⚪ Posiciones cerradas</span><span class="bsum">${lst.length} cerradas · P/G neta <b class="${netoT>=0?'pos':'neg'}">${netoT>=0?'+':''}${fmt(netoT)}</b> (incl. dividendos) · «Archivar» fija el ciclo</span></div><div class="pos-blk-b"><div style="overflow:auto;padding:2px 2px 4px"><table><thead>${head}</thead><tbody>${rows}${sub}</tbody></table></div></div></div>`;
  if(!el._invClosedBound){ el._invClosedBound=true; el.addEventListener('click',function(e){ if(e.target.closest('[data-ficha],[data-archive],[data-delcerrada]'))return; var h=e.target.closest('.pos-blk-h'); if(h){ var b=h.parentElement; b.classList.toggle('open'); window._invClosedOpen=b.classList.contains('open'); } }); }
}
